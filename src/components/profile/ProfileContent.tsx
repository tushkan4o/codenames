...
export default function ProfileContent({ profileId }: ProfileContentProps) {
    ...
    api.getCluesByUser(profileId).then((clues) => {
      ...
      clues.forEach((clue) => {
        api.getClueStats(clue.id).then((s) => {
          setClueStatsMap((prev) => ({ ...prev, [clue.id]: { attempts: s.attempts, avgScore: s.avgScore, ratingsCount: s.ratingsCount ?? 0, avgRating: s.avgRating ?? 0, clueRating: s.clueRating ?? 0 } }));
        });
      });
    });
      ...
      entries.forEach((entry) => {
        if (entry.clue) {
          api.getClueStats(entry.clue.id).then((s) => {
            setClueStatsMap((prev) => ({ ...prev, [entry.clue!.id]: { attempts: s.attempts, avgScore: s.avgScore, ratingsCount: s.ratingsCount ?? 0, avgRating: s.avgRating ?? 0, clueRating: s.clueRating ?? 0 } }));
          });
        }
      });
    });
  ...
  const sortedGiven = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      else if (givenSort === 'clueRating') diff = (clueStatsMap[b.id]?.clueRating ?? 0) - (clueStatsMap[a.id]?.clueRating ?? 0);
  ...
  const sortedSolved = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      if (!solvedSort || solvedSort === 'date') diff = b.result.timestamp - a.result.timestamp;
        ...
        const aSR = a.result.solveRating ?? 0;
        const bSR = b.result.solveRating ?? 0;
        diff = bSR - aSR;
        ...
        <div className="flex items-start gap-4 mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!isOwnProfile && user && (<>
                    <button
                      ...
                      disabled={subLoading}
                      className={`p-1.5 rounded-lg transition-all shrink-0 ${subscribed ? 'text-board-blue bg-board-blue/15' : 'text-gray-500 hover:text-gray-300 hover:bg-gray-700/50'}`}
                      title={subscribed ? t.profile.unsubscribe : t.profile.subscribe}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill={subscribed ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {stats && (
            <div className="w-16 h-16 rounded-lg border-2 border-gray-600 shrink-0 flex flex-col items-center justify-center mr-6">
              <span className="text-amber-400 text-2xl leading-none" style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontWeight: 800 }}>{(stats.overallRating ?? 0) > 0 ? stats.overallRating : '–'}</span>
                ...
                return (
                  <div key={i}>
                    <div
                      ...
                      className={`bg-gray-800/60 border rounded-lg px-4 py-1.5 cursor-pointer transition-colors hover:border-gray-600 ${isExpanded ? 'border-gray-500' : 'border-gray-700/30'}`}
                    >
                      <div className={`grid ${isOwnProfile ? 'grid-cols-[1fr_3rem_2rem_2rem] sm:grid-cols-[1fr_9rem_3rem_3.5rem_2rem_2rem]' : 'grid-cols-[1fr_3.5rem_2rem_2rem] sm:grid-cols-[1fr_9rem_3.5rem_2rem_2rem]'} gap-x-2 items-center`}>
                        <span className="font-bold text-white uppercase text-sm truncate">
